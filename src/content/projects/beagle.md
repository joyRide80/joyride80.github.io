---
title: "Beagle"
headline: "Beagle"
subtitle: "Creating More Compelling Proposals, Faster"
year: "2014-2015"
url: ""
tags:
  - "Product design"
  - "Lo-fi & Hi-fi prototype"
  - "User interview"
  - "Visual design"
  - "UI design"
category: "SaaS"
order: 4
thumbnail: "/images/projects/beagle/sculpture.jpg"
heroImages:
  - src: "/images/projects/beagle/four-screens.png"
    caption: "The four key screens"
    size: "large"
  - src: "/images/projects/beagle/overview-screen.png"
    caption: "Overview screen"
    size: "large"
  - src: "/images/projects/beagle/beagle-logo.jpg"
    caption: "The final Beagle logo"
    size: "small"
---

## Introduction

Beagle is an online proposal writing tool designed and crafted specifically for digital agencies. Beagle provides a way to create, share, revise and send beautiful proposals easily. We built Beagle by using Podio API. Currently, the project has been discontinued. However, you can see some ideas and sketches at the provided links.

Great work starts with great proposal

![Paper sculpture for pre-launch](/images/projects/beagle/sculpture.jpg)

![Logo sketches](/images/projects/beagle/logo-sketch.png)

![The final Beagle logo](/images/projects/beagle/beagle-logo.jpg)

We later on decided on the name Beagle. So I made this Origami Beagle for the pre-launch event. I made this paper sculpture in an abstract form since we didn't know at that time what the product would be.

## Background

Around October 2014, some of the existing Podio team, including me, started a small project technically built on Podio Platform. It took a couple of months before we came up with Beagle. Creating Beagle from scratch requires a lot of research and brainstorming. We arranged numerous interviews with digital agencies in Copenhagen and London. We also performed qualitative and quantitative user testing to understand our target users.

## Modular design concept

Writing a proposal is a tedious activity. Agencies sent out several proposals per day. Most of them share the same patterns or content. We look at this pain point and see if our technology can solve their problems. Our solution is to let agencies create their proposal using a reusable block of content. We aimed to make it easy for team members to edit the content and track the flow. They can touch up the looking and feel of the proposal they made using our Design Studio feature. All of this flow is finished in one place and on browsers.

## How Beagle works

There are four main screens in Beagle — An overview screen where writers can work with other team members and see all the proposal statuses. Then, on a detailed screen, they can focus on their content. Here, they can pick or replace each section from the section screen. Finally, with a design studio screen, they can craft the look and feel of their beautiful proposal.

![The four key screens](/images/projects/beagle/four-screens.png)

![Overview screen](/images/projects/beagle/overview-screen.png)

![Section screen](/images/projects/beagle/section-screen.png)

![Design studio screen](/images/projects/beagle/design-studio.png)
